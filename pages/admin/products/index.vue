<template>
  <div>
    ...
    <div class="flex items-center justify-between mb-8">
      ...
      <button class="btn-neon-primary text-xs flex items-center gap-2" @click="openCreate">
          ...
          >
            <td class="px-4 py-3">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <p class="font-orbitron text-xs text-white">{{ p.name }}</p>
                  <p class="text-xs text-gray-600 font-rajdhani">{{ p.id }}</p>
                </div>
              </div>
            </td>
            ...
            <td class="px-4 py-3">
              <div class="flex items-center justify-end gap-2">
                <button
                  class="text-xs font-orbitron px-3 py-1.5 text-neon-cyan transition-all hover:bg-neon-cyan/10"
                  style="border: 1px solid rgba(0,245,255,0.3);"
                  @click="openEdit(p)"
                >EDIT</button>
                <button
                  class="text-xs font-orbitron px-3 py-1.5 text-neon-pink transition-all hover:bg-neon-pink/10"
    ...
    <Transition name="fade">
      ...
      >
        <div
          class="w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6"
        ...
        >
          <div class="flex items-center justify-between mb-6">
            ...
            <button class="text-gray-500 hover:text-white" @click="showModal = false">✕</button>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            ...
            <div class="col-span-2 flex gap-6">
              <label class="flex items-center gap-2 text-sm font-rajdhani text-gray-400 cursor-pointer">
                <input v-model="form.featured" type="checkbox" class="accent-purple-500" />
                Featured
              </label>
              <label class="flex items-center gap-2 text-sm font-rajdhani text-gray-400 cursor-pointer">
                <input v-model="form.bestseller" type="checkbox" class="accent-pink-500" />
                Bestseller
              </label>
              <label class="flex items-center gap-2 text-sm font-rajdhani text-gray-400 cursor-pointer">
                <input v-model="form.newArrival" type="checkbox" class="accent-cyan-500" />
                New Arrival
              </label>
            </div>
          </div>
          ...
          <div class="flex gap-3 mt-6">
            <button class="btn-neon-primary flex-1 text-xs" :disabled="saving" @click="saveProduct">
              {{ saving ? 'SAVING...' : (editingProduct ? 'SAVE CHANGES' : 'CREATE PRODUCT') }}
            </button>
            <button class="btn-neon-secondary text-xs px-6" @click="showModal = false">CANCEL</button>
          </div>
        </div>
      </div>
    </Transition>
      ...
      >
        <div class="w-full max-w-sm p-6 text-center" style="background: #0f0f1a; border: 1px solid rgba(255,45,120,0.5);">
          ...
          <div class="flex gap-3">
            <button
              class="flex-1 text-xs font-orbitron px-4 py-3 text-neon-pink transition-all hover:bg-neon-pink/10"
              style="border: 1px solid rgba(255,45,120,0.5);"
              :disabled="saving"
              @click="deleteProduct"
            >{{ saving ? 'DELETING...' : 'DELETE' }}</button>
            <button class="flex-1 btn-neon-secondary text-xs" @click="deletingProduct = null">CANCEL</button>
          </div>
        </div>
      </div>
    </Transition>
